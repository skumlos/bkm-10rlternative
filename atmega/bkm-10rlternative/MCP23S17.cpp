...
/*
 * Edited to work with SPI1 instead for Atmega328PB, 2022, Martin Hejnfelt (martin@hejnfelt.com)
 */

#include <SPI1.h>                 // Arduino IDE SPI library - uses AVR hardware SPI features
...
void MCP::begin() {
  ...
  SPI1.begin();                          // Start up the SPI bus
...
void MCP::byteWrite(uint8_t reg, uint8_t value) {      // Accept the register and byte
  ...
  SPI1.transfer(OPCODEW | (_address << 1));             // Send the MCP23S17 opcode, chip address, and write bit
  SPI1.transfer(reg);                                   // Send the register we want to write
  SPI1.transfer(value);                                 // Send the byte
...
void MCP::wordWrite(uint8_t reg, unsigned int word) {  // Accept the start register and word 
  ...
  SPI1.transfer(OPCODEW | (_address << 1));             // Send the MCP23S17 opcode, chip address, and write bit
  SPI1.transfer(reg);                                   // Send the register we want to write 
  SPI1.transfer((uint8_t) (word));                      // Send the low byte (register address pointer will auto-increment after write)
  SPI1.transfer((uint8_t) (word >> 8));                 // Shift the high byte down to the low byte location and send
  ::digitalWrite(_ss, HIGH);                           // Take slave-select high
}
...
unsigned int MCP::digitalRead(void) {       // This function will read all 16 bits of I/O, and return them as a word in the format 0x(portB)(portA)
  ...
  ::digitalWrite(_ss, LOW);                 // Take slave-select low
  SPI1.transfer(OPCODER | (_address << 1));  // Send the MCP23S17 opcode, chip address, and read bit
  SPI1.transfer(GPIOA);                      // Send the register we want to read
  value = SPI1.transfer(0x00);               // Send any byte, the function will return the read value (register address pointer will auto-increment after write)
  value |= (SPI1.transfer(0x00) << 8);       // Read in the "high byte" (portB) and shift it up to the high location and merge with the "low byte"
  ::digitalWrite(_ss, HIGH);                // Take slave-select high
...
uint8_t MCP::byteRead(uint8_t reg) {        // This function will read a single register, and return it
  ...
  ::digitalWrite(_ss, LOW);                 // Take slave-select low
  SPI1.transfer(OPCODER | (_address << 1));  // Send the MCP23S17 opcode, chip address, and read bit
  SPI1.transfer(reg);                        // Send the register we want to read
  value = SPI1.transfer(0x00);               // Send any byte, the function will return the read value
  ::digitalWrite(_ss, HIGH);                // Take slave-select high
